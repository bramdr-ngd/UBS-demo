...
parser = argparse.ArgumentParser(description='Generate DemoPersona Entity AccountRole data for Lily demo environments')
...
# Outputfile
file_out=open(output_file("DemoPersonaAccountRoleData.csv"), 'w')

# Create header
headers = open(output_file('AccountRoleData.hdr'), "r").readlines()[0]
file_hdr=open(output_file("DemoPersonaAccountRoleData.hdr"), 'w')
file_hdr.write(headers)

def main_crm():
    for i in range(1, 15):
        text_file = open(output_file('AccountRoleData.csv'), "r")
